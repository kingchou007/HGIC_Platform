...
for idx, t in enumerate(key_frames):
    ...
    for i in range(0, trajectories.shape[1], 2):
        ...
        for j in range(1, len(x)):
            ax.plot(x[j-1:j+1], y[j-1:j+1], color=cmap(j/len(x)), linewidth=2)
        
        ax.scatter(x[-1], y[-1], s=10, zorder=2)
        circle_radius = 20
        circle_radius_2 = 7
        ax.scatter(x[-1], y[-1], s=np.pi * (circle_radius ** 2), color='silver', alpha=0.5, zorder=1)
        ax.scatter(x[-1], y[-1], s=np.pi * (circle_radius_2 ** 2), color='lightgreen', alpha=0.5, zorder=1)

...
for i in range(0, trajectories.shape[1], 2):
    ...
    for t in range(time_interval, len(x), time_interval):
        ...
        if t == 510:
            circle_radius = 20
            circle_radius_2 = 7
            ax.scatter(x[t], y[t], s=np.pi * (circle_radius ** 2), color='silver', alpha=0.2, zorder=1)
            ax.scatter(x[t], y[t], s=np.pi * (circle_radius_2 ** 2), color='lightgreen', alpha=0.2, zorder=1)

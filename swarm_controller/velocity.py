...
class VelocityComputation():
    def __init__(self):
        ...
        self.trajectories = [[[]
                              for _ in range(600)] for _ in range(self.num_uavs)]
        self.t = 0
    ...
    def compute_repulsion_force(self, pos_i, pos_j, repulsion_distance, safe_distance_uav, K_rep):
        ...
        if distance < repulsion_distance:
            ...
            repulsion_vector = -K_rep * \
                (repulsion_distance - distance) * (r_ij/distance)
        else:
    ...
    def compute_velocity(self, rep_dis, safe_dis, add_rep):
        for i in range(self.num_uavs):
            ...
            self.v_cmd[:, i:i + 1] = self.v_sep + \
                self.v_coh + self.v_rep + v_mig
            if np.linalg.norm(self.v_cmd[:, i:i + 1]) > self.v_max:
                self.v_cmd[:, i:i + 1] = self.v_max * self.v_cmd[:,
                                                                 i:i + 1] / np.linalg.norm(self.v_cmd[:, i:i + 1])
    ...
    def form_V(self, rep_dis, safe_dis, spacing=8):
        ...
        for t in range(700):
                ...
                for j in range(self.num_uavs):
                    if j != i:
                        ...
                        if dist_ij < self.r_max:
                            ...
                            # Add repulsion force to the repulsion velocit
                            v_rep += repulsion_force
                            v_sep += self.compute_separation_force(
                                pos_i, pos_j)
                            v_coh += self.compute_cohesion_force(pos_i, pos_j)
                v_rep /= N_i
                v_sep /= N_i
            ...
            for i in range(self.num_uavs):
                name_i = "UAV" + str(i + 1)
                self.client.moveByVelocityZAsync(
                    self.v_cmd[0, i], self.v_cmd[1, i], self.z_cmd, 0.1, vehicle_name=name_i)
    ...
    def space_ccupation(self):
        ...
        for t in range(600):
            ...
            drone_positions = np.random.rand(
                n_drones, 2) * [x_max - x_min, y_max - y_min] + [x_min, y_min]
                ...
                for j in range(self.num_uavs):
                    if j != i:
                        ...
                        if np.linalg.norm(pos_j - pos_i) < self.r_max:
                            ...
                            self.v_rep += 2 * \
                                np.mean(v_rep, axis=1, keepdims=True)
    ...
    def save_trajectory(self):
        ...
        for i in range(self.num_uavs):
            ...
            writer.writerow(
                [name_i, position.x_val, position.y_val, position.z_val])

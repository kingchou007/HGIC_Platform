...
class VelocityComputation:
    ...
    def __init__(self):
        ...
        self.trajectories = [[[] for _ in range(600)] for _ in
                             range(self.num_uavs)]
        self.t = 0
    ...
    def get_UAV_pos(self, vehicle_name='SimpleFlight'):
        state = \
            self.client.simGetGroundTruthKinematics(vehicle_name=vehicle_name)
    ...
    def get_avg_altitude(self):
        ...
        swarm_altitude = \
            [self.client.getMultirotorState(vehicle_name='UAV' + str(i
             + 1)).kinematics_estimated.position.z_val for i in
             range(self.num_uavs)]
    ...
    def compute_repulsion_force(
        ...
        if distance < repulsion_distance:
            ...
            repulsion_vector = -K_rep * (repulsion_distance - distance) \
                * (r_ij / distance)
        else:
    ...
    def compute_forces(
        ...
        if norm < self.r_max:
            ...
            if add_rep:
                v_rep = self.compute_repulsion_force(pos_i, pos_j,
                                                     rep_dis, safe_dis, self.k_rep)
    ...
    def compute_velocity(
        ...
        for i in range(self.num_uavs):
            ...
            for j in range(self.num_uavs):
                if j != i:
                    ...
                    if np.linalg.norm(pos_j - pos_i) < self.r_max:
                        ...
                        (v_sep, v_coh, v_rep) = \
                            self.compute_forces(pos_i, pos_j, rep_dis,
                                                safe_dis, add_rep)
                        self.v_sep += v_sep
                        self.v_coh += v_coh
                        self.v_rep += v_rep
            ...
            self.v_cmd[:, i:i + 1] = self.v_sep + self.v_coh \
                + self.v_rep + v_mig
            if np.linalg.norm(self.v_cmd[:, i:i + 1]) > self.v_max:
                self.v_cmd[:, i:i + 1] = self.v_max * self.v_cmd[:, i:i
                                                                 + 1] / np.linalg.norm(self.v_cmd[:, i:i + 1])

    def move_UAVs(self, z_cmd):
        for i in range(self.num_uavs):
            name_i = 'UAV' + str(i + 1)
            self.client.moveByVelocityZAsync(self.v_cmd[0, i],
                                             self.v_cmd[1, i], z_cmd, 0.1, vehicle_name=name_i)
    ...
    def point_generator(self, type, spacing):
        ...
        if type == 'circle':
            ...
            for i in range(self.num_uavs):
                ...
                yield group_center + spacing \
                    * np.array([[np.cos(formation_angle)],
                               [np.sin(formation_angle)]])
        elif type == 'line':
            ...
            for i in range(self.num_uavs):
                yield group_center + np.array([[abs(i - self.num_uavs
                                                    // 2) * spacing], [(i - self.num_uavs // 2)
                                                                       * spacing]])
            ...
            for j in range(self.num_uavs):
                if j != i:
                    ...
                    if dist_ij < self.r_max:
                        ...
                        repulsion_force = \
                            self.compute_repulsion_force(pos_i, pos_j,
                                                         rep_dis, safe_dis, self.k_rep)

                        # Add repulsion force to the repulsion velocit

                        v_rep += repulsion_force
                        v_sep += self.compute_separation_force(pos_i,
                                                               pos_j)
                        v_coh += self.compute_cohesion_force(pos_i,
                                                             pos_j)
    ...
    def form_V(
        ...
        trajectories = [[[] for _ in range(700)] for _ in
                        range(self.num_uavs)]
        velocities = [[[] for _ in range(700)] for _ in
                      range(self.num_uavs)]
        for t in range(700):
                ...
                for j in range(self.num_uavs):
                    if j != i:
                        ...
                        if dist_ij < self.r_max:
                            ...
                            repulsion_force = \
                                self.compute_repulsion_force(pos_i,
                                                             pos_j, rep_dis, safe_dis,
                                                             self.k_rep)
                            ...
                            v_rep += repulsion_force
                            v_sep += \
                                self.compute_separation_force(pos_i,
                                                              pos_j)
                            v_coh += self.compute_cohesion_force(pos_i,
                                                                 pos_j)
                v_rep /= N_i
    ...
    def form_line(
        ...
        trajectories = [[[] for _ in range(800)] for _ in
                        range(self.num_uavs)]
        velocities = [[[] for _ in range(800)] for _ in
                      range(self.num_uavs)]
        for t in range(800):
    ...
    def get_all_UAV_positions(self):
        ...
        for i in range(self.num_uavs):
            name_i = 'UAV' + str(i + 1)
            positions[i, :] = \
                self.get_UAV_pos(vehicle_name=name_i).flatten()
        return positions
    ...
    def circle_move_circle(self):
        trajectories = [[[] for _ in range(600)] for _ in
                        range(self.num_uavs)]
        velocities = [[[] for _ in range(600)] for _ in
                      range(self.num_uavs)]
        for t in range(600):
            angle = 2 * np.pi * t / 600
            group_center_radius = 60
            group_center = group_center_radius \
            ...
            for i in range(self.num_uavs):
                ...
                formation_angle = formation_angle_offset * i
                formation_point = group_center + formation_radius \
                    * np.array([[np.cos(formation_angle)],
                               [np.sin(formation_angle)]])
                ...
                for j in range(self.num_uavs):
                    if j != i:
                        ...
                        if np.linalg.norm(pos_j - pos_i) < self.r_max:
                            (v_sep, v_coh, v_rep) = \
                                self.compute_forces(pos_i, pos_j, 10,
                                                    5, True)
                            self.v_sep += v_sep
                            self.v_coh += v_coh
                            self.v_rep += v_rep
    ...
    def V_move_circle(self):
        ...
        for t in range(600):
            ...
            formation_points = []
            for i in range(self.num_uavs):
                point = np.array([[abs(i - self.num_uavs // 2)
                                 * spacing], [(i - self.num_uavs // 2)
                                 * spacing]])

                # Compute the rotation matrix based on the current angle

                formation_angle = angle - np.pi / 2
                rotation_matrix = np.array([[np.cos(formation_angle),
                                             -np.sin(formation_angle)],
                                            [np.sin(formation_angle),
                                             np.cos(formation_angle)]])
            ...
            for (i, formation_point) in enumerate(formation_points):
                ...
                for j in range(self.num_uavs):
                    if j != i:
                        ...
                        if np.linalg.norm(pos_j - pos_i) < self.r_max:
                            (v_sep, v_coh, v_rep) = \
                                self.compute_forces(pos_i, pos_j, 8, 3,
                                                    True)
                            self.v_sep += v_sep
                            self.v_coh += v_coh
                            self.v_rep += v_rep * 2
    ...
    def space_ccupation(self):
        ...
        for t in range(600):
            ...
            drone_positions = np.random.rand(n_drones, 2) * [x_max
                                                             - x_min, y_max - y_min] + [x_min, y_min]
            ...
            for i in range(self.num_uavs):
                ...
                if not -1 in vor.regions[region]:
                    polygon = [vor.vertices[j] for j in
                               vor.regions[region]]
                ...
                for j in range(self.num_uavs):
                    if j != i:
                        ...
                        if np.linalg.norm(pos_j - pos_i) < self.r_max:
                            ...
                            self.v_sep += np.mean(v_sep, axis=1,
                                                  keepdims=True)
                            self.v_coh += np.mean(v_coh, axis=1,
                                                  keepdims=True)
                            self.v_rep += 2 * np.mean(v_rep, axis=1,
                                                      keepdims=True)
                ...
                self.v_cmd[:, i:i + 1] = self.v_sep + self.v_coh \
                    + self.v_rep + v_mig
                if np.linalg.norm(self.v_cmd[:, i:i + 1]) > self.v_max:
                    self.v_cmd[:, i:i + 1] = self.v_max * self.v_cmd[:,
                                                                     i:i + 1] / np.linalg.norm(self.v_cmd[:, i:i
                                                                                                          + 1])
            ...
            for i in range(self.num_uavs):
                ...
                with open('so_trajectories.csv', 'w', newline='') as \
                        file:
                    writer = csv.writer(file)
                    for i in range(self.num_uavs):
                        writer.writerow(['UAV' + str(i + 1) + '_X',
                                         'UAV' + str(i + 1) + '_Y'])
    ...
    def line_search(self, spacing=15):
        ...
        initial_formation_points = [group_center + np.array([[i
                                    * spacing], [0]]) for i in
                                    range(self.num_uavs)]
        ...
        for t in range(600):
            ...
            for (i, initial_formation_point) in \
                    enumerate(initial_formation_points):
                formation_point = initial_formation_point \
                    + v_center_to_target
    ...
    def get_collision_info(self):
        for i in range(self.num_uavs):
            ...
            collision_info = \
                self.client.simGetCollisionInfo(vehicle_name=name)
    ...
    def save_trajectory(self):
        ...
        for i in range(self.num_uavs):
            ...
            writer.writerow([name_i, position.x_val, position.y_val,
                            position.z_val])
